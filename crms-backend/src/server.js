...
app.use(cors({
  origin: (origin, cb) => {
    // Allow: no origin (server-to-server), file:// (null), listed origins, or ALL if no list set
    if (!origin || origin === 'null') return cb(null, true);
    if (!allowedOrigins.length)       return cb(null, true); // open in dev if no CORS_ORIGIN set
    if (allowedOrigins.includes('*')) return cb(null, true);
    if (allowedOrigins.includes(origin)) return cb(null, true);
    // Also allow any localhost/127.0.0.1 regardless of port
